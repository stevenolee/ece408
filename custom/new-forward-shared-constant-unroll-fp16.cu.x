...
#define x2d(i1, i0) x_shared[(i1) * W_x + (i0)]
    ...
    __half2 *x_shared = &shared[0];
    __half2 *k_shared = &shared[W_x * H_x];

    __half2 res = __float2half2_rn(0.0);
    __half2 temp = __float2half2_rn(0.0);
    for (size_t c = 0; c < C; ++c) {
      ...
      for (size_t p = 0; p < K; p+=2) { 
        for (size_t q = 0; q < K; q+=2) {
          temp = __hmul2(x2d(h_thread + p, w_thread + q), k2d(p, q));
          res = __hadd2(temp, res);
          if (p+1 < K) {
            temp = __hmul2(x2d(h_thread + p + 1, w_thread + q), k2d(p + 1, q));
            res = __hadd2(temp, res);
          }
          if (q+1 < K) {
            temp = __hmul2(x2d(h_thread + p, w_thread + q + 1), k2d(p, q + 1));
            res = __hadd2(temp, res);
          }
          if (p+1 < K && q+1 < K) {
            temp = __hmul2(x2d(h_thread + p + 1, w_thread + q + 1), k2d(p + 1, q + 1));
            res = __hadd2(temp, res);
          }
        }
      }
