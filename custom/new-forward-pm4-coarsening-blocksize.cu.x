...
#define x2d(i1, i0) x_shared[(i1) * W_x + (i0)]
    ...
    const size_t w_thread_odd = threadIdx.x + 1;
    const size_t h = h_base + h_thread;
    const size_t w = w_base + w_thread;
    const size_t w_odd = w_base + w_thread_odd;
    ...
    if (threadIdx.x % 2 == 0){
      bool next_exists = (threadIdx.x + 1) < (blockDim.x);
      for (size_t c = 0; c < C; ++c) {
        ...
        if (next_exists && h_thread < K && w_thread_odd < K)
          k2d(h_thread, w_thread_odd) = k4d(m, c, h_thread, w_thread_odd);
        ...
        for (size_t i = h_thread; i < H_x; i += blockDim.y){
          for (size_t j = w_thread; j < W_x; j += blockDim.x){
            ...
            if (next_exists)
              x2d(i, j+1) = x4d(b, c, h_base + i, w_base + j+1);
          }
        }
        ...
        if (h < H_out && w < W_out) {
          ...
          for (size_t p = 0; p < K-1; p+=2) { 
            for (size_t q = 0; q < K-1; q+=2) {
              ...
              if (odd_bool && next_exists){
                res_odd += x2d(h_thread + p, w_thread_odd + q) * k2d(p, q);
                res_odd += x2d(h_thread + p+1, w_thread_odd + q) * k2d(p+1, q);
                res_odd += x2d(h_thread + p, w_thread_odd + q+1) * k2d(p, q+1);
                res_odd += x2d(h_thread + p+1, w_thread_odd + q+1) * k2d(p+1, q+1);
              }
  #else
              ...
              if (odd_bool && next_exists){
                res_odd += x4d(b, c, h + p, w_odd + q) * k4d(m, c, p, q);
                res_odd += x4d(b, c, h + p+1, w_odd + q) * k4d(m, c, p+1, q);
                res_odd += x4d(b, c, h + p, w_odd + q+1) * k4d(m, c, p, q+1);
                res_odd += x4d(b, c, h + p+1, w_odd + q+1) * k4d(m, c, p+1, q+1);
              }
  #endif
            }
          }
          if (K % 2 == 1) {
            for (size_t i = 0; i < K - 1; i +=2) {
              ...
              if (odd_bool && next_exists){
                res_odd += x2d(h_thread + i, w_thread_odd + K - 1) * k2d(i, K - 1);
                res_odd += x2d(h_thread + i + 1, w_thread_odd + K - 1) * k2d(i + 1, K - 1);
                res_odd += x2d(h_thread + K - 1, w_thread_odd + i) * k2d(K - 1, i);
                res_odd += x2d(h_thread + K - 1, w_thread_odd + i + 1) * k2d(K - 1, i + 1);
              }
  #else
              ...
              if (odd_bool && next_exists){
                res_odd += x4d(b, c, h + i, w_odd + K - 1) * k4d(m, c, i, K - 1);
                res_odd += x4d(b, c, h + i + 1, w_odd + K - 1) * k4d(m, c, i + 1, K - 1);
                res_odd += x4d(b, c, h + K - 1, w_odd + i) * k4d(m, c, K - 1, i);
                res_odd += x4d(b, c, h + K - 1, w_odd + i + 1) * k4d(m, c, K - 1, i + 1);
              }
  #endif
            }
  #ifdef SHARED
            ...
            if (odd_bool && next_exists)
              res_odd += x2d(h_thread + K - 1, w_thread_odd + K - 1) * k2d(K - 1, K - 1);
  #else
            res += x4d(b, c, h + K - 1, w + K - 1) * k4d(m, c, K - 1, K - 1);
            if (odd_bool && next_exists)
              res_odd += x4d(b, c, h + K - 1, w_odd + K - 1) * k4d(m, c, K - 1, K - 1);
  #endif
          }
        }
      }
      ...
      if (h < H_out && w < W_out)
        y4d(b, m, h, w) = res;
      if (h < H_out && w_odd < W_out)
        y4d(b, m, h, w_odd) = res_odd;
